...
import MyDatePicker from "./datetimePicker/MyDatePicker";
      ...
      case "price":
        return (
          <InputNumber
            ...
            placeholder={`Nhập ${field.label}`}
            formatter={
              (value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ".") // format 100000 -> 100.000
            }
            parser={(value) => value!.replace(/\./g, "")} // xoá dấu . khi lưu
          />
        );
      case "date":
        return <MyDatePicker />;
      case "textarea": // 👈 hỗ trợ ghi chú
        return (
          <Input.TextArea
            rows={4}
            placeholder={`Nhập ${field.label}`}
